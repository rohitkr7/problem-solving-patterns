...
class Solution {
    ...
    public static void main(String[] args) {
        System.out.println("Topological sort: " + topologicalSort(4, new int[][] { { 3, 2 }, { 3, 0 }, { 2, 0 }, { 2, 1 } }));
        //Following are the two valid topological sorts for the given graph:
        //1) 3, 2, 0, 1
        //2) 3, 2, 1, 0
        System.out.println("Topological sort: " + topologicalSort(5, new int[][] { { 4, 2 }, { 4, 3 }, { 2, 0 }, { 2, 1 }, { 3, 1 } }));
        ...
        System.out.println("Topological sort: " + topologicalSort(7, new int[][] { { 6, 4 }, { 6, 2 }, { 5, 3 }, { 5, 4 }, { 3, 0 }, { 3, 1 }, { 3, 2 }, { 4, 1 } }));
        // Following are all valid topological sorts for the given graph:
        // 1) 5, 6, 3, 4, 0, 1, 2
        // 2) 6, 5, 3, 4, 0, 1, 2
        // 3) 5, 6, 4, 3, 0, 2, 1
...
class Solution {
    public static boolean isSchedulingPossible(int tasks, int[][] prerequisites) {
        ...
        //1. initialize the graph
        //count incoming edges
        int[] steps = new int[tasks];
        //adjacency list graph
        List<List<Integer>> graph = new ArrayList<>(tasks);
    ...
    public static void main(String[] args) {
        System.out.println("Is scheduling possible: " + isSchedulingPossible(3, new int[][] { { 0, 1 }, { 1, 2 } }));
        //true
        ...
        System.out.println("Is scheduling possible: " + isSchedulingPossible(3, new int[][] { { 0, 1 }, { 1, 2 }, { 2, 0 } }));
        // false
        //The tasks have a cyclic dependency, therefore they cannot be scheduled.

        System.out.println("Is scheduling possible: " + isSchedulingPossible(6, new int[][] { { 0, 4 }, { 1, 4 }, { 3, 2 }, { 1, 3 } }));
        // true
...
class Solution {
    ...
    public static void main(String[] args) {
        System.out.println("Is scheduling possible: " + findOrder(3, new int[][] { { 0, 1 }, { 1, 2 } }));
        //To execute task '1', task '0' needs to finish first. 
        //Similarly, task '1' needs to finish before '2' can be scheduled. 
        //One possible scheduling of tasks is: [0, 1, 2] 

        System.out.println("Is scheduling possible: " + findOrder(3, new int[][] { { 0, 1 }, { 1, 2 }, { 2, 0 } }));
        // []
        //The tasks have a cyclic dependency, therefore they cannot be scheduled.

        System.out.println("Is scheduling possible: " + findOrder(6, new int[][] { { 2, 5 }, { 0, 5 }, { 0, 4 }, { 1, 4 }, { 3, 2 }, { 1, 3 } }));
        // [0, 1, 4, 3, 2, 5]
        //A possible scheduling of tasks is: [0 1 4 3 2 5] 
    }
}
...
class Solution {
    public static void printOrders(int tasks, int[][] prerequisites) {
        List<Integer> possibleSchedules = new ArrayList<>();
    ...
    private static void printAllTopologicalSorts(List<List<Integer>> graph, int[] steps, List<Integer> sources, List<Integer> schedule) {
        //4. For each source, add to schedule and decrement child steps
        //if child === 0, add to source queue
        if (!sources.isEmpty()) {
            for (int i = 0; i < sources.size(); i++) {
                int vertex = sources.get(i);
                schedule.add(vertex);

                //clone current sources
                List<Integer> sourcesForNextCall = new ArrayList<>(sources);

                //only remove the current source
                //all other sources will remain in queue for next call
                sourcesForNextCall.remove(Integer.valueOf(vertex));
                ...
                for (int p = 0; p < graph.get(vertex).size(); p++) {
                    steps[graph.get(vertex).get(p)]++;
                }
            }
        }
        ...
        if (schedule.size() == steps.length) System.out.println(schedule);
    }

    public static void main(String[] args) {
        printOrders(3, new int[][] { { 0, 1 }, { 1, 2 } });
        // [0, 1, 2]
        // There is only possible ordering of the tasks.

        printOrders(4, new int[][] { { 3, 2 }, { 3, 0 }, { 2, 0 }, { 2, 1 } });
        // 1) [3, 2, 0, 1]
        // 2) [3, 2, 1, 0]
        // There are two possible orderings of the tasks meeting all prerequisites.

        printOrders(6, new int[][] { { 2, 5 }, { 0, 5 }, { 0, 4 }, { 1, 4 }, { 3, 2 }, { 1, 3 } });
        // 1) [0, 1, 4, 3, 2, 5]
        // 2) [0, 1, 3, 4, 2, 5]
        // 3) [0, 1, 3, 2, 4, 5]
        // 4) [0, 1, 3, 2, 5, 4]
...
class Solution {
    public static String findOrder(String[] words) {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            for (int j = 0; j < Math.min(w1.length(), w2.length()); j++) {
                ...
                if (parent != child) {
                    ...
                    graph.get(parent).add(child);
                    //increment childs step
                    steps.put(child, steps.get(child) + 1);
        ...
        for (Map.Entry<Character, Integer> entry : steps.entrySet()) {
            if (entry.getValue() == 0) {
        ...
        StringBuilder sortedDictionary = new StringBuilder();
        while (!sources.isEmpty()) {
    ...
    public static void main(String[] args) {
        System.out.println("Character order: " + findOrder(new String[] { "ba", "bc", "ac", "cab" }));
        //bac
        ...
        System.out.println("Character order: " + findOrder(new String[] { "cab", "aaa", "aab" }));
        //cab
        //From the given words we can conclude the following ordering among its characters:
        ...
        System.out.println("Character order: " + findOrder(new String[] { "ywx", "wz", "xww", "xz", "zyy", "zwz" }));
        //ywxz
        //From the given words we can conclude the following ordering among its characters:
...
class Solution {
    public static boolean canConstruct(int[] originalSeq, int[][] sequences) {
        ...
        if (steps.size() != originalSeq.length) return false;
        ...
        while (!sources.isEmpty()) {
            ...
            if (originalSeq[reconstructed.size()] != sources.peek()) {
            ...
            for (int child : graph.get(vertex)) {
                //get the node's children to decrement thier steps
                steps.put(child, steps.get(child) - 1);
                if (steps.get(child) == 0) sources.add(child);
            }
        }

        // if reconstructed's size !== orginalSeq's size
        //there is no way to construct
        return reconstructed.size() == originalSeq.length;
    }

    public static void main(String[] args) {
        System.out.println("Can construct: " + canConstruct(new int[] { 1, 2, 3, 4 }, new int[][] { { 1, 2 }, { 2, 3 }, { 3, 4 } }));
        //true
        ...
        System.out.println("Can construct: " + canConstruct(new int[] { 1, 2, 3, 4 }, new int[][] { { 1, 2 }, { 2, 3 }, { 2, 4 } }));
        //false
        ...
        //1) [1, 2, 3, 4]
        //2) [1, 2, 4, 3]
        System.out.println("Can construct: " + canConstruct(new int[] { 3, 1, 4, 2, 5 }, new int[][] { { 3, 1, 5 }, { 1, 4, 2, 5 } }));
        //true
...
class Solution {
    public static List<Integer> findTrees(int nodes, int[][] edges) {
        ...
        if (nodes == 1) return Arrays.asList(0);
    ...
    public static void main(String[] args) {
        System.out.println("Roots of MHTs: " + findTrees(5, new int[][] { { 0, 1 }, { 1, 2 }, { 1, 3 }, { 2, 4 } }));
        //[1, 2]
        ...
        System.out.println("Roots of MHTs: " + findTrees(4, new int[][] { { 0, 1 }, { 0, 2 }, { 2, 3 } }));
        //[0, 2]
        ...
        System.out.println("Roots of MHTs: " + findTrees(4, new int[][] { { 1, 2 }, { 1, 3 } }));
        //[1]
    }
}
````
